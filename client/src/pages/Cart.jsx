...
export default function ProductDetails(props) {
  ...
  const increaseQuantity = async (id) => {
    if (props.UserCarts.find(cart => cart.id === id).order_quantity < props.Products.find(product => product.id === props.UserCarts.find(cart => cart.id === id).product_id).stock) {
      try {
          ...
          <Row className="justify-content-between align-items-start px-0">
            <Col xs={12} lg={7}>
            {
              props.UserCarts.filter(cart => cart.user_id === props.User.id).length > 0 ? (
                props.UserCarts.filter(cart => cart.user_id === props.User.id).map((item) => (
                  <Col id={`${item.id}`} key={item.id} xs={12} className={`py-4 px-0 mb-4 animate__animated animate__slideInLeft`} style={{ borderTop:"1px solid #613D2B",borderBottom:"1px solid #613D2B", backgroundColor: props.darkMode ? "#F6E6DA" : "transparent" }}>
